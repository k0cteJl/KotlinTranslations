...
У каждого отрендеренного компонента italic по умолчанию явно выключен. Иначе Minecraft-клиент
рендерит компонент, используемый как display name/lore предмета, курсивом — без всякой связи с
тем, что просила разметка `.lang`-значения. Это fallback, а не переопределение, так что явный
тег `<italic>`/`<i>` (или legacy `&o`) в исходнике по-прежнему применяется как обычно.
